            ...
            <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
                <md-card>
                    <md-card-header data-background-color="green">
                        <h4 class="title">GROUPWISE MALE/FEMALE BIRD'S BODY WEIGHT AT 5TH WEEK</h4>
                    </md-card-header>
                    <md-card-content>
                        <md-table v-model="users" :table-header-color="tableHeaderColor">
                            <!-- <md-table-row>
                                <md-table-head>Group</md-table-head>
                                <md-table-head>Total Birds</md-table-head>
                                <md-table-head>Male Birds</md-table-head>
                                <md-table-head>Female Birds</md-table-head>
                                <md-table-head>Wt</md-table-head>
                                <md-table-head>FCR</md-table-head>
                            </md-table-row> -->
                            <md-table-row slot="md-table-row" slot-scope="{ item }">
                                <md-table-cell md-label="Group">{{ item.Group }}</md-table-cell>
                                <md-table-cell md-label="Total Birds">{{ item.Total_birds }}</md-table-cell>
                                <md-table-cell md-label="Male Birds">{{ item.Male }}</md-table-cell>
                                <md-table-cell md-label="Female Birds">{{ item.Female }}</md-table-cell>
                                <md-table-cell md-label="Wt">{{ item.FCR }}</md-table-cell>
...
export default {
    ...
    data() {
        return {
            ...
            users: [
                { Group: "T1", Total_birds: "144", Male: "98", Female: "49", FCR: " " },
                { Group: "T2", Total_birds: "142", Male: "75", Female: "67", FCR: " " },
                { Group: "T3", Total_birds: "148", Male: "77", Female: "71", FCR: " " },
                { Group: "T4", Total_birds: "147", Male: "79", Female: "68", FCR: " " },
            ],
        };
    },
    methods: {
